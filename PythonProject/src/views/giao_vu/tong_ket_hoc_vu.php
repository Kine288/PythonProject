...
async function loadWarnings() {
    ...
    const res = await fetch('../../../api/gpa.php?action=warnings&hoc_ky_id=' + encodeURIComponent(hocKyId));
...
document.getElementById('btn-recalculate').addEventListener('click', async () => {
    try {
        const hocKyId = hocKyEl.value;
        const res = await fetch('../../../api/gpa.php?action=recalculate', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({hoc_ky_id: hocKyId})
        });
        const data = await res.json();
        if (!data.success) {
...
document.getElementById('btn-student-result').addEventListener('click', async () => {
    try {
        ...
        const hocKyId = hocKyEl.value;
        const url = `../../../api/gpa.php?action=student_result&sinh_vien_id=${encodeURIComponent(svId)}&hoc_ky_id=${encodeURIComponent(hocKyId)}`;
