...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if ($isEdit) {
		...
		$res = ['success' => false, 'message' => 'Chi ADMIN moi co quyen tao sinh vien.'];
	}
	...
	<div class="p-margin">
		<div class="flex flex-col lg:flex-row lg:items-end justify-between gap-4 mb-lg">
			<div>
				...
				<p class="text-body-md text-on-surface-variant max-w-2xl">Quan ly thong tin co ban va lop hoc cho sinh vien.</p>
			</div>
			<a class="flex items-center gap-2 border border-primary text-primary px-5 py-2.5 rounded-lg font-semibold hover:bg-primary-container/10 transition-colors" href="danh_sach.php">
		...
		<div class="bg-white rounded-xl p-6 border border-slate-100 shadow-sm">
			<form method="post">
				<div class="grid grid-cols-1 md:grid-cols-2 gap-sm">
					...
					<?php if (!$isEdit): ?>
						<div>
							<label class="block text-label-md font-label-md text-secondary mb-1.5 ml-1">Mat khau khoi tao</label>
							<input class="w-full h-11 border-outline-variant rounded-lg focus:border-primary focus:ring-1 focus:ring-primary text-body-md" type="text" name="mat_khau" value="123456">
						</div>
					<?php endif; ?>
				</div>

				<div class="flex flex-wrap gap-2 mt-6">
					<button class="flex items-center gap-2 bg-primary text-on-primary px-6 py-3 rounded-lg font-title-lg hover:shadow-lg transition-all active:scale-[0.98] disabled:opacity-60" type="submit" <?php echo (!$isEdit && !$canCreate) ? 'disabled' : ''; ?>>
						<span class="material-symbols-outlined" data-icon="check_circle">check_circle</span>
						<?php echo $isEdit ? 'Luu thay doi' : 'Tao sinh vien'; ?>
					</button>
					<a class="flex items-center gap-2 border border-outline-variant text-secondary px-6 py-3 rounded-lg font-semibold hover:bg-slate-50 transition-colors" href="danh_sach.php">
